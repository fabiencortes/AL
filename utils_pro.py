...
# ======================================================
# 📥 MAIL → NAVETTE (PRO) — parsing robuste (FR/EN)
# ======================================================
...
def _normalize_french_date_to_ddmmyyyy(s: str) -> str:
    ...
    try:
        ...
        if m2:
            ...
            mon_u = mon_u.replace("É","E").replace("È","E").replace("Ê","E").replace("Ë","E")
            mon_u = mon_u.replace("À","A").replace("Â","A").replace("Ä","A")
            mon_u = mon_u.replace("Ô","O").replace("Ö","O")
            mon_u = mon_u.replace("Û","U").replace("Ü","U").replace("Ù","U")
            mon_u = mon_u.replace("Ç","C")
...
def parse_mail_to_navette_pro(text: str) -> dict:
    """
    Parse robuste pour mails entreprise (FR).
    Retour : {"MODE":"MAIL","TRANSFERS":[{...}]}
    """
    ...
    # 1) Essayer parseurs existants
    try:
        d2 = parse_mail_to_navette_v2(text)
        if isinstance(d2, dict) and d2.get("MODE") in ("MAIL","TABLE") and (d2.get("TRANSFERS") or d2.get("ROWS")):
            return d2
    except Exception:
        pass

    try:
        d1 = parse_mail_to_navette_flexible(text)
        if isinstance(d1, dict) and d1:
            tr = {}
            for k in ["DATE","HEURE","PAX","VOL","ADRESSE","DESTINATION","RAW"]:
                if k in d1 and d1.get(k) not in (None, ""):
                    tr[k] = d1.get(k)
            if tr:
                return {"MODE":"MAIL","TRANSFERS":[tr]}
    except Exception:
        pass

    def _grab_after_label(label: str) -> str:
    ...
    secteur = _grab_after_label("SECTEUR")
    sbu = _grab_after_label("S.B.U.") or _grab_after_label("SBU")
    demandeur = _grab_after_label("Demandeur")
    voyageur = _grab_after_label("Voyageur") or _grab_after_label("Voyageur(s)")
    gsm = _grab_after_label("N° GSM") or _grab_after_label("N° GSM du voyageur") or _grab_after_label("GSM")
    imputation = _grab_after_label("Imputation") or _grab_after_label("BDC") or _grab_after_label("Num BDC")

    date_txt = _grab_after_label("Date")
    date_norm = _normalize_french_date_to_ddmmyyyy(date_txt) if date_txt else ""
    if not date_norm:
        for ln in lines:
            if re.search(r"\b\d{1,2}\s+[A-Za-zÉÈÊËÀÂÄÔÖÛÜÙÇéèêëàâäôöûüùç]+\s+\d{2,4}\b", ln):
                date_norm = _normalize_french_date_to_ddmmyyyy(ln)
                if date_norm:
                    break

    # Heure : extraire la première heure trouvée après 'Heure' (ou dans le texte)
    heure = ""
    hline = ""
    for i, ln in enumerate(lines):
        if ln.upper().startswith("HEURE"):
            hline = ln
            if i + 1 < len(lines) and ln.strip().endswith(":"):
                hline = lines[i + 1]
            break
    m_h = re.search(r"\b(\d{1,2})\s*[:hH]\s*(\d{2})\b", hline) if hline else None
    if not m_h:
        for ln in lines:
    ...
    pickup = _grab_after_label("Lieu de pick up") or _grab_after_label("Lieu de pickup") or _grab_after_label("Pickup")
    dest = _grab_after_label("Transfert à") or _grab_after_label("Transfert a") or _grab_after_label("Destination")

    # Destination multi-lignes (Domicile + adresse + CP)
    if not dest:
        for i, ln in enumerate(lines):
            if ln.upper().startswith("TRANSFERT"):
                chunks = []
                for j in range(i+1, min(i+6, len(lines))):
                    if re.match(r"^[A-ZÉÈÊËÀÂÄÔÖÛÜÙÇ ]+\s*:$", lines[j]):
                        break
    ...
    transfer = {}
    if date_norm:
        transfer["DATE"] = date_norm
    if heure:
        transfer["HEURE"] = heure
    if voyageur:
        transfer["NOM"] = voyageur
    if gsm:
        transfer["Tél"] = gsm
    if pickup:
    ...
    if dest:
        transfer["DEST"] = dest
        transfer["ADRESSE"] = dest

    meta_parts = []
    if secteur:
        meta_parts.append(f"SECTEUR: {secteur}")
    if sbu:
        meta_parts.append(f"SBU: {sbu}")
    if demandeur:
        meta_parts.append(f"Demandeur: {demandeur}")
    if imputation:
        transfer["Num BDC"] = imputation
        meta_parts.append(f"Imputation: {imputation}")
    if meta_parts:
        transfer["REMARQUE"] = " | ".join(meta_parts)
